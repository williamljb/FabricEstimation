...
"""
def align(gt_abscloth, rotmat, midhip):
    tr = pytorch3d.transforms.Rotate(rotmat).cuda()
    return tr.transform_points(gt_abscloth - midhip)
def align_1(gt_abscloth, rotmat, midhip):
    tr = pytorch3d.transforms.Rotate(rotmat.permute(0,2,1)).cuda()
    return tr.transform_points(gt_abscloth) + midhip

def sample_cores(gt_abscloth, batch_size, num_fps, num_sample):
    dim = gt_abscloth.shape[2]
    num_tot = gt_abscloth.shape[1]+1
...
def sample_patches(gt_abscloth, coresout, batch_size, num_fps, num_sample):
    # knn from scipy
    dim = gt_abscloth.shape[2]
    ind = []
    cores = coresout.reshape(batch_size, 3, num_fps).permute(0,2,1)
...
def batch_rodrigues_back(rot):
    tmp = (rot - torch.transpose(rot, 1, 2)) / 2
    v = tmp.reshape([-1, 9])[:,[7,2,3]]
    sin = torch.norm(v, dim=1).unsqueeze(-1)
    cos = ((rot[:,0,0]+rot[:,1,1]+rot[:,2,2]).unsqueeze(-1) - 1) / 2
...
def estimate_translation(S, joints_2d, focal_length=5000., img_size=224.):
    ...
    S = S[:, 25:, :].cpu().numpy() * np.array([[[1.,-1,-1]]])
