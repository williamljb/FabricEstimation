...
class BaseDataset(Dataset):
    ...
    def pose_processing(self, pose, r, f):
        ...
        if f:
            pose = flip_pose(pose)
            pose_befrot = flip_pose(pose_befrot)
        # (72),float
        pose = pose.astype('float32')
        pose_befrot = pose_befrot.astype('float32')
        return pose_befrot, pose
    ...
    def __getitem__(self, index):
        ...
        item['map'] = torch.from_numpy(self.mapname[index]).float() - trans # only used when doing mgn!

        pose_befrot, pose_aftrot = self.pose_processing(pose, rot, flip)
        item['pose'] = torch.from_numpy(pose_aftrot).float()
        item['pose_befrot'] = torch.from_numpy(pose_befrot).float()
