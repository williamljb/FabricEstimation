...
from utils.geometry import batch_rodrigues, batch_rodrigues_back, align
import trimesh
...
if __name__ == '__main__':
    ...
    model = nn.DataParallel(baseline_hmr(config.SMPL_MEAN_PARAMS, smpl=smpl)).to(device)
    check_baseline = False
    if check_baseline:
        extra = nn.DataParallel(baseline_hmr(config.SMPL_MEAN_PARAMS, smpl=smpl)).to(device)
    ...
    if check_baseline:
        state_dict = torch.load('data/model_checkpoint.pt')['model']
        new_state_dict = OrderedDict()
        for k, v in state_dict.items():
            if not k.startswith('module.'):
                name = 'module.'+k
                ...
                name = k
            new_state_dict[name] = v
        extra.load_state_dict(new_state_dict, strict=False)
    ...
    with torch.no_grad():
        ...
        if check_baseline:
            pred_rotmat, pred_betas, pred_camera, _ = extra(norm_img.to(device))
            pred_rotmat[:,0,1:3,:] = -pred_rotmat[:,0,1:3,:]
        # pred_rotmat=batch_rodrigues(gt_pose.reshape([-1,3])).view(1, 24, 3, 3)
        # pred_betas=gt_shape
        pred_output = smpl(betas=pred_betas, body_pose=pred_rotmat[:,1:], global_orient=pred_rotmat[:,0].unsqueeze(1), pose2rot=False)
        gt_out = smpl(betas=gt_shape, body_pose=gt_rotmat[:,1:], global_orient=gt_rotmat[:,0].unsqueeze(1), pose2rot=False)
        gt_vert = gt_out.vertices
        pred_vertices = pred_output.vertices
        # if True:
        #     import pytorch3d.transforms
        #     from utils.geometry import align
        #     pred_vertices = align(gt_vert, gt_rotmat[:,:1].reshape(1, 3, 3), gt_out.joints[:,8:9,:])
        #     tr = pytorch3d.transforms.RotateAxisAngle(1.6).cuda()
        #     pred_vertices = tr.transform_points(pred_vertices)+gt_out.joints[:,8:9,:]
        print((pred_output.joints-gt_out.joints).norm(dim=2).mean())
        pred_rotmat, pred_betas = gt_rotmat, gt_shape
    ...
    #visualize cloth
        with open('demoout/ours_cloth.obj', 'w') as f:
            for p in pred_abscloth[0].cpu().numpy():
                f.write('v {} {} {}\n'.format(p[0],p[1],p[2]))
        gt_cloth, gt_norm = load_points('../sim_data_CH/{}/{:04d}_*.obj'.format(nam,ind), 16384)
        gt_cloth = (gt_cloth - a['trans'][ind]).float()
        if dim == 6:
            gt_cloth = torch.cat([gt_cloth, gt_norm], dim=2)
        gt_cloth = gt_cloth.cuda()
        gt_cloth = align(gt_cloth, gt_rotmat[:,:1].reshape(1, 3, 3), gt_out.joints[:,8:9,:])
        # with open('demoout/gt_cloth.obj', 'w') as f:
        #     for p in gt_cloth.reshape(-1,dim).cpu().numpy():
        #         f.write('v {} {} {}\n'.format(p[0],p[1],p[2]))
        gt_abscloth, rec,rec_g, gt_g,gt_l = decoder(gt_cloth, None, torch.cat([gt_rotmat.reshape(1,24*9)[:,9:],gt_shape],dim=1))
        # # rec, rec_g = decoder.module.decode(gt_g, gt_l, 
        # #     torch.cat([pred_rotmat.reshape(1,24*9)[:,9:],pred_betas],dim=1))
        # rec = rec.permute(0,2,3,1).reshape(1,-1,dim)
        rec_g = rec_g.permute(0,2,3,1)
        # gt_abscloth = gt_abscloth.permute(0,2,3,1)+rec_g.permute(0,2,1,3)
        # with open('demoout/gt_cloth_ae.obj', 'w') as f:
        #     for p in rec.detach().reshape(-1,dim).cpu().numpy():
        #         f.write('v {} {} {}\n'.format(p[0],p[1],p[2]))
        print(chamfer_distance(rec_g.reshape(1,-1,dim),pred_cores)[0])
        # print(chamfer_distance(rec,pred_abscloth)[0])
        print(chamfer_distance(gt_cloth,pred_abscloth)[0])
        geomloss = SamplesLoss(loss="sinkhorn", p=2, blur=.05) #chamfer_distance #
        print(geomloss(pred_abscloth.reshape(1,-1,dim), gt_cloth.reshape(1,-1,dim)))
        # print(nn.MSELoss()(z_g,gt_g))
        # print(nn.MSELoss()(z_l,gt_l))

    # Calculate camera parameters for rendering
    pred_vertices = pred_vertices[0].cpu().numpy()
    camera_translation = torch.stack([pred_camera[:,1], pred_camera[:,2], 2*constants.FOCAL_LENGTH/(constants.IMG_RES * pred_camera[:,0] +1e-9)],dim=-1)
    if check_baseline:
        camera_translation[:,1:] = camera_translation[:,1:] + pred_output.joints[:,8,1:]*2
