...
class baseline_HMR(nn.Module):
    ...
    def __init__(self, block, layers, smpl_mean_params, smpl):
        ...
        self.new_m = ['fc_cloth01','fc_cloth02','fc_cloth1','drop3','deccloth']
    ...
    def forward(self, x, init_pose=None, init_shape=None, init_cam=None, n_iter=3):
        ...
        pred_cam = init_cam
        pred_cloth = torch.zeros([batch_size,self.dim_cloth]).cuda()
        ...
        for i in range(n_iter):
            xc = self.fc_cloth01(xf) + pred_cloth
...
def baseline_hmr(smpl_mean_params, pretrained=True, **kwargs):
    """ Constructs an HMR model with ResNet50 backbone.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = baseline_HMR(Bottleneck, [3, 4, 6, 3],  smpl_mean_params, **kwargs)
    if pretrained:
