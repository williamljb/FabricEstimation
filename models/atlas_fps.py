...
class Mapping2Dto3D(nn.Module):
    ...
    def __init__(self, latent_size, dim, hidden):
        ...
        self.conv1 = torch.nn.Conv2d(dim, self.bottleneck_size, 1)
...
class PointNet(nn.Module):
    def __init__(self, nlatent=1024, nf=3, nc=3):
        ...
        self.nf = nf
        self.conv0 = torch.nn.Conv2d(nf, nc, 1)
...
class Atlas(nn.Module):
    ...
    def __init__(self, nfps, nsample, nr, dim):
        ...
        self.de_l = Atlasnet(nsample+1, nr, nf=latent_g+latent_l, dim=dim, hidden=256)
        self.nsample = nsample
        self.nfps = nfps

        self.apply(weights_init)

    def forward(self, pts, cores, body, train=True):
        # pts: bs*3*nfps*nsample
        # cores: bs*3*1*nfps
        # body: bs*226
        # print('atlas:',pts.shape,cores.shape,body.shape)
        bdy_rep = body.unsqueeze(2).repeat(1,1,self.nfps).unsqueeze(2) # bs*226*1*nfps
        # pts_g = torch.cat([cores,bdy_rep],dim=1) # bs*(3+226)*1*nfps
        z_g=self.en_g(cores,bdy_rep) # bs*512*1
        # rec_g = self.de_g(torch.cat([z_g,body.unsqueeze(2)],dim=1)) # bs*3*1*nfps
        # return None, rec_g

        cores_l = cores.permute(0,1,3,2).repeat(1,1,1,self.nsample) # bs*3*nfps*nsample
        z_l=self.en_l(pts, cores_l) # bs*512*nfps
        ...
        #print(p)
        return rec, rec_g.permute(0,1,3,2)
...
class AtlasDecoder(nn.Module):
    ...
    def __init__(self, nfps, nsample, nr, dim):
        ...
        self.de_l = Atlasnet(nsample, nr, nf=latent_g+latent_l+dim, dim=dim, hidden=128)
        self.nsample = nsample
        self.nfps = nfps

        self.apply(weights_init)

    def forward(self, z_g, z_l, body, train=True):
        # z_g: bs*512*1
        # z_l: bs*8*nfps
        # body: bs*226
        # print('atlas:',pts.shape,cores.shape,body.shape)
        rec_g = self.de_g(torch.cat([z_g,body.unsqueeze(2)],dim=1)) # bs*3*1*nfps
        rec_l = self.de_l(torch.cat([z_g.repeat(1,1,self.nfps),z_l, rec_g.reshape(z_g.shape[0],self.dim,self.nfps)],dim=1)) # bs*3*nfps*nsample
        rec = rec_l + rec_g.permute(0,1,3,2) # bs*3*nfps*nsample

        #print(p)
        return rec.reshape(rec.shape[0],3,self.nfps*self.nsample)
